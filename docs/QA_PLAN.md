...
Validate that the MVP proves the daily quest habit loop without hiding math or reward defects behind AI wording. Sprint 1 passes QA only when a child can create a profile, complete "Reignite the Fraction Furnace," receive deterministic feedback and rewards, buy one forge upgrade, and a parent can see a summary based on real attempts.

## Sprint 1 Release Gates

- Daily quest contains exactly 8 problems: 5 focus, 2 review, and 1 challenge.
...
5. Confirm the quest sequence has 5 focus problems, 2 review problems, and 1 challenge problem.
6. Answer the first problem correctly and confirm immediate positive feedback appears before moving on.
7. Answer a later problem incorrectly and confirm the app shows non-shaming feedback plus a retry path.
8. Request hint level 1 and confirm it is a conceptual nudge that does not reveal the final answer.
9. Retry the missed problem and submit the correct answer.
10. Complete all 8 problems and finish the quest.
...
Use deterministic expected answers. AI text can vary, but correctness, hints, explanations, and scoring must not.

| Area | Check | Expected Result |
| --- | --- | --- |
| Equivalent fractions | Generate or encounter an equivalent fraction problem. | Correct answer matches a mathematically equivalent fraction; alternate equivalent forms are accepted only if the answer checker supports them intentionally. |
...
| Review problem | Encounter one non-fraction review item. | Skill metadata identifies it as review, not focus. |
| Challenge problem | Encounter the challenge item. | Difficulty and reward treatment match challenge metadata. |
...
- Problem prompts, answer checking, explanations, and hints still render from deterministic content.
...
- Math answers are deterministic and spot-checked.
...
- Quest completion cannot happen before all 8 problems are answered.
- API responses do not expose hidden correct-answer metadata to the child problem display.
...
- `GET /quests/:questId/next-problem` returns one unanswered problem at a time without server-only explanation metadata.
...
- `POST /quests/:questId/complete` returns quest summary and earned rewards only after all 8 problems are answered.
